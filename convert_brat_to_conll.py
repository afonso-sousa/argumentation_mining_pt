"""
Script adapted from https://github.com/UKPLab/acl2017-neural_end2end_am/blob/master/progs/Brat2ConllConverter/convert_brat_conll.py
"""
# %%
import os
import sys
...
path = os.getcwd()
# infolder = sys.argv[1]
infolder = 'data/AM/persuasive/test'
...
# %%
# Ignore brat config files
for folder in [element for element in folders if '.conf' not in element]:
    ...
    for listing in set(files):
        ...
        if 'conll' in extension:
            ...
            annotations = [element.split('\t')
                           for element in annotation.split('\n')]
            annotation_marks = [element[1]
                                for element in annotations if len(element) > 1]
            ...
            for annot in annotation_marks:
                ...
                try:
                    ...
                    annotation_dict[label] = []
                # if label == 'Stance' or label == 'supports': # Paragraph Level
                #    continue
